...
1. **Compositing is refused.** A gate used to decide whether there was room to take the 8 KB RGBA scratch; when it said no, the fallback decoded each frame to RGB with **alpha dropped**, so an animation whose frames are partial or transparent painted **black** where it should have shown what was underneath — reported from the panel as intermittent unrendered pixels. **Resolved** by reserving the scratch at boot and deleting the gate, so compositing is unconditional.
2. **A frame fails to decode** — `frame 1/2 decode failed (out of memory)`. The heap was too chopped for libwebp's working set at that instant, so that frame is missing and the animation renders **partially**.
...
**And the composite scratch is reserved at boot, which is what makes compositing unconditional.** Two things forced that. The first is the workload: the server serves **hundreds of unrelated apps** — stills, animations, scrolls, of very different weight — so any threshold on available memory is a threshold some app trips, and a tripped app renders with alpha dropped, silently and permanently. Determinism matters more than optimality here. The second is that the gate was measuring a self-inflicted wound: at 8 KB, allocated per fetch, the scratch lands wherever there is room and **splits in half** the very block the next frame's decode needs (the largest free run read 24,576 with it released and 17,408–26,624 with it held). Taken at boot instead, off a 139 KB block before WiFi or the HTTP server have run, it costs the run-time heap *nothing* — `largest` is unchanged at 139,264 across the reservation — and the decodes stopped being a judgment call. Two commits were spent tuning that gate before measuring where the scratch was actually landing.
