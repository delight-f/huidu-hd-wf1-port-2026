...
- **Resolved by 6 instead of 8, plus the gate change below.** 6 is a native CIE table depth, so `depth_sim` shows it monotonic and correct, and it is ~9 KB cheaper than 8. The top-level `CMakeLists.txt` now injects `PIXEL_COLOR_DEPTH_BITS=6` into the library so the macro and the run-time value cannot drift apart again — the drift was the whole bug.
...
3. **Flash the depth-6 + gated-composite build and check three things.** (a) Colours stay true. (b) `frame 1 decode failed` is gone — the depth-8 run produced 16 in 3.5 minutes. (c) The composite gate either stops refusing, or refuses with a largest-run figure that genuinely fails the 22 KB test rather than at 37,9xx free against a total-free threshold. Both changes are built and host-verified and have never run on a board.
4. **If a decode still fails, read the gate's own message and `largest_internal`** — the gate now prints the largest run it measured, which is the honest number. What would remain is genuine fragmentation rather than a bad threshold, and then the payload wants a fixed boot-reserved buffer so each fetch stops carving a fresh hole.
