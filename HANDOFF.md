...
4. **What is left is headroom, not bugs.** `largest_internal` at run time sits at 22,528–23,552 against the 21,320 a frame needs. In order of honesty: size the assets at the server, which is where the problem belongs; or trim what is reserved at boot now that three buffers are held there; or accept the 16 KB ceiling as the boundary it is. **Do not lower the colour depth** — see trap 11; that lever costs more than it looks and corrupts colour.
