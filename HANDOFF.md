...
3. **Done and verified: depth 6 with a matched CIE table, and compositing made unconditional by reserving the scratch at boot.** Colours are true, there were **zero** compositing refusals in a 3.3-minute window (was ~30), and 22/22 samples were stable. See §3.
4. **What is left is the fetch's own churn.** Two `frame 1 decode failed` remain, at `free 37028 largest 16384` — plenty free, not enough contiguous, at the moment the payload and the scratch coexist. The next move is to give the payload a fixed, boot-reserved buffer too, by the same argument that just worked for the scratch: taken at boot it comes off a 139 KB block for free, and at run time it stops carving a fresh hole every ten seconds. Watch the cost — payload plus scratch plus canvas is ~28 KB of the boot 43 KB — and if that is too much, size it to what the server actually sends (most bodies are 218–1,316 B; the heavy ones are ~13 KB).
